...
<div class="doc-date" data-anchor-pagination="#docList">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      <?php foreach($docGroupMonth as $docMonth): ?>
       <?php foreach($allDoc as $key => $doc): ?>
         <?php if(convertDateYearMonth($doc['date_doc']) == convertDateYearMonth($docMonth['date_doc'])): ?>
            <!--Выводит год-->
            <?php if(convertDateYear($allDoc[$key - 1]['date_doc']) != convertDateYear($docMonth['date_doc'])): ?>
            <tr class="title text-bold text-center">
              <td colspan="7"><?= convertDateYear($docMonth['date_doc']); ?></td>
            </tr>
            <?php endif; ?>
            <!--Выводит месяц-->
            <?php if(convertDateYearMonth($allDoc[$key - 1]['date_doc']) != convertDateYearMonth($docMonth['date_doc']) and $doc['date_doc'] != 0): ?>
            <tr class="title text-bold">
              <td colspan="7"><?= showNameMonth($docMonth['date_doc']); ?></td>
            </tr>
            <?php endif; ?>
