...
<div class="doc-date" data-anchor-pagination="#docList">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      <?php foreach($docGroupMonthUpload as $docMonth): ?>
       <?php foreach($allDoc as $key => $doc): ?>
         <?php if(convertDateYearMonth($doc['date_uploads']) == convertDateYearMonth($docMonth['date_uploads'])): ?>
            <!--Выводит год-->
            <?php if(convertDateYear($allDoc[$key - 1]['date_uploads']) != convertDateYear($docMonth['date_uploads'])): ?>
            <tr class="title text-bold text-center">
              <td colspan="7"><?= convertDateYear($docMonth['date_uploads']); ?></td>
            </tr>
            <?php endif; ?>
            <!--Выводит месяц-->
            <?php if(convertDateYearMonth($allDoc[$key - 1]['date_uploads']) != convertDateYearMonth($docMonth['date_uploads']) and $doc['date_uploads'] != 0): ?>
            <tr class="title text-bold">
              <td colspan="7"><?= showNameMonth($docMonth['date_uploads']); ?></td>
            </tr>
            <?php endif; ?>
