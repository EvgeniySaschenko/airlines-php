<div class="notice">
  <!--Add-->
  <div data-notice="#noticeFlightAssignmentUserCrewAdd" data-ancor="userCrew" class="hidden alert alert-success" role="alert">
    <?= $word[147]['name_'.$lang]; ?>
  </div>
  <!--Edit-->
  <div data-notice="#noticeFlightAssignmentUserCrewEdit" data-ancor="userCrew" class="hidden alert alert-success" role="alert">
    <?= $word[180]['name_'.$lang]; ?>
  </div>
</div>

<!--Добавить пользователя в экипаж-->
<div class="a-flight-assignment-user-crew-add">
  <form role="form" method="post" action="insert.php?lang=<?= $lang ;?>&id_section=<?= $getIdSection ;?>&id_subsection=<?= $getIdSubsection ;?>&id_flight_assignment=<?= $getIdFlightAssignment;?>&action=user_assignment_flight">
    <table class="table table-bordered">
      <caption class="text-bold text-center"><?= $word[211]['name_'.$lang]; ?></caption>
      <tbody>
        <tr>
          <td>
            <?php dropDownListUsersSectionFL('id_user', $allUserSortLastNameNoHide, '', 4); ?>
          </td>
        </tr>
        <!--Отправить-->
        <tr>
...
<!--Добавить пользователя в экипаж-->
<div class="a-flight-assignment-user-crew-edit">
  <form role="form" method="post" action="update.php?lang=<?= $lang ;?>&id_section=<?= $getIdSection ;?>&id_subsection=<?= $getIdSubsection ;?>&id_flight_assignment=<?= $getIdFlightAssignment;?>&action=user_assignment_flight">
    <table class="table table-striped table-align-middle">
      ...
      <tbody>
        ...
        <tr>
          <!--Должность-->
          <td>
            <?= dropDownListRquired('id_rank[]', $allRank, $userFlight['id_rank_user']); ?>
          </td>
          <!--Имя-->
          <td>
